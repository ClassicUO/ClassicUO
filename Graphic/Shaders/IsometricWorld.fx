...
{
	return float4( input.Depth, put.Depth, put.Depth, 1.0f );
}
...
{	
	...
	float4 color = tex2D(DrawSampler, .TexCoord);
	if (color.a <= 0)
		clip(-1);
	...
	//(color.r + color.g + color.b) / 3
		float4 hueColor = tex2D(HueSampler0, float2(  color.r  , .Hue.x / HuesPerTexture));
...
float4 PixelShader_MiniMap(PS_INPUT IN) : COLOR0
{
	...
	{
		color = tex2D(MiniMapSampler, .Normal);
	}
