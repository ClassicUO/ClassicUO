...
namespace ClassicUO.Game.Views
{
    public class TileView : View
    {
        ...
        {
            new Point(0, -1), new Point(1, -1), new Point(-1, 0), new Point(1, 0), new Point(2, 0), new Point(-1, 1),
            new Point(0, 1), new Point(1, 1), new Point(2, 1), new Point(0, 2), new Point(1, 2)
        };
        ...
        private void UpdateStreched(Facet map)
        {
            ...
            {
                surroundingTilesZ[i] = map.GetTileZ((short) (GameObject.Position.X + _surroundingIndexes[i].X),
                    (short) (GameObject.Position.Y + _surroundingIndexes[i].Y));
            }
            ...
            _normals[0] = CalculateNormal(surroundingTilesZ[2], surroundingTilesZ[3], surroundingTilesZ[0],
                surroundingTilesZ[6]);
            _normals[1] = CalculateNormal(GameObject.Position.Z, surroundingTilesZ[4], surroundingTilesZ[1],
                surroundingTilesZ[7]);
            _normals[2] = CalculateNormal(surroundingTilesZ[5], surroundingTilesZ[7], GameObject.Position.Z,
                surroundingTilesZ[9]);
            _normals[3] = CalculateNormal(surroundingTilesZ[6], surroundingTilesZ[8], surroundingTilesZ[3],
                surroundingTilesZ[10]);
            ...
            if (_vertex[0].Hue != hue) _vertex[0].Hue = _vertex[1].Hue = _vertex[2].Hue = _vertex[3].Hue = hue;
        }

        private static Vector3 CalculateNormal(float a, float b, float c, float d) =>
            Vector3.Normalize(new Vector3(a - b, 1f, c - d));
    }
}