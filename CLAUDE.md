...
Built components deploy one folder per mod to `ecs-mods/<mod>/{mod.json, mod.wasm}` (copied next to the exe; the host scans `<exe>/ecs-mods/*/mod.json`). The `mod.json` manifest names the mod (`name`, `version`), the `wasm` file to load, and a reserved `ruleset` object (`ModManifest` in `TinyEcs.Bevy.Modding`). Examples in `src/Mods/`, each with its own `wit/`: `ecs-topbar` / `ecs-status` / `ecs-netlog` (Rust), `ecs-ui` (TypeScript, jco/componentize-js React).
