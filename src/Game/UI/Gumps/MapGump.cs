...
namespace ClassicUO.Game.UI.Gumps
{
    class MapGump : Gump
    {
        ...
        public MapGump(Serial serial, ushort gumpid, int width, int height) : base(serial, 0)
        {
            ...
            AddChildren(new ResizePic(0x1432)
            {
                Width = width + 44, Height = height + 61
            });


            AddChildren(_buttons[0] = new Button((int)ButtonType.PlotCourse, 0x1398, 0x1398) { X = (width - 100) / 2, Y = 5, ButtonAction = ButtonAction.Activate,  });
            AddChildren(_buttons[1] = new Button((int)ButtonType.StopPlotting, 0x1399, 0x1399) { X = (width - 70) / 2, Y = 5, ButtonAction = ButtonAction.Activate, });
            AddChildren(_buttons[2] = new Button((int)ButtonType.ClearCourse, 0x139A, 0x139A) { X = (width - 66) / 2, Y = height + 37, ButtonAction = ButtonAction.Activate, });
            ...
            AddChildren(_textureControl = new TextureControl()
            {
                X = 24, Y = 31,
                Width = width,
                Height = height
            ...
            AddChildren(new GumpPic(width - 20, height - 20, 0x0139D, 0));
