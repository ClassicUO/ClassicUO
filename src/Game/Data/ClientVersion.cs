...
namespace ClassicUO.Data
{
    internal enum ClientVersion
    {
        ...
        CV_306E =
            (3 << 24) | (0 << 16) | (0 << 8) |
            ('e' & 0xFF), // Adds a packet with the client type, switches to mp3 from midi for sound files
        ...
        CV_308Z =
            (3 << 24) | (0 << 16) | (8 << 8) |
            ('z' & 0xFF), // Age of Shadows. Adds paladin, necromancer, custom housing, resists, profession selection window, removes save password checkbox
        ...
        CV_500A =
            (5 << 24) | (0 << 16) | (0 << 8) |
            ('a' & 0xFF), // Paperdoll buttons journal becomes quests, chat becomes guild. Use mega FileManager.Cliloc. Removes verdata.mul.
        CV_5020 = (5 << 24) | (0 << 16) | (2 << 8) | 0, // Adds buff bar
        CV_5090 = (5 << 24) | (0 << 16) | (9 << 8) | 0, //
        CV_6000 =
            (6 << 24) | (0 << 16) | (0 << 8) |
            0, // Adds colored guild/all chat and ignore system. New targeting systems, object properties and handles.
            ...
            if (!string.IsNullOrEmpty(versionText))
            {
                ...
                if (int.TryParse(buff[0], out int major) && major >= byte.MinValue && major <= byte.MaxValue)
                {
                    ...
                    if (int.TryParse(buff[1], out int minor) && minor >= byte.MinValue && minor <= byte.MaxValue)
                    {
                        ...
                        {
                            if (!(int.TryParse
                                (buff[extra_index], out build) && build >= byte.MinValue && build <= byte.MaxValue))
                            {
                                return false;
                            }
                        ...
                        if (!(int.TryParse
                                  (buff[extra_index].Substring(0, i), out int num_extra) &&
                              num_extra >= byte.MinValue &&
                              num_extra <= byte.MaxValue))
                        {
                            return false;
                        }
