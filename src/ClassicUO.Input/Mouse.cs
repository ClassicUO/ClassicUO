...
namespace ClassicUO.Input
{
    public static class Mouse
    {
        ...
        public static void Update(nint windowHandle, int backBufferWidth, int backBufferHeight, int clientWidth, int clientHeight, float dpiScale)
        {
            if (!MouseInWindow)
            {
                SDL.SDL_GetGlobalMouseState(out float x, out float y);
                SDL.SDL_GetWindowPosition(windowHandle, out int winX, out int winY);
            ...
            Position.X = (int) ((double) Position.X * (backBufferWidth / clientWidth) / dpiScale);
            Position.Y = (int) ((double) Position.Y * (backBufferHeight / clientHeight) / dpiScale);
