...
namespace ClassicUO.Game.Managers
{
    internal class AutoLootManager
    {
                ...
                Task.Factory.StartNew(() =>
                {
                    ...
                    try
                    {
                        ...
                        if (lootItems != null && !lootItems.IsEmpty)
                        {
                            ...
                            {
                                Item m = _world.Items.Get(moveItem);
                                if (m != null)
                                {
                                    GameActions.GrabItem(_world, m, m.Amount);
                                    Task.Delay(delay).Wait();
                                }
                            }
                        }
        ...
        public void CheckAndLoot(Item i)
        {
            ...
            if (IsOnLootList(i))
            {
                GameActions.Print(_world, $"SAL Looting: {i.Name} {i.Graphic} x {i.Amount}");
                lootItems.Enqueue(i);
            }
        }
        ...
        private void Load()
        {
            Task.Factory.StartNew(() =>
            {
                ...
                else
                {
                    ...
                    {
                        GameActions.Print(_world, "There was an error loading your auto loot config file, please check it with a json validator.", 32);
                        loaded = false;
                    }
