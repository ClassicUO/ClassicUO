...
internal struct UIRenderingPlugin : IPlugin
{
    public void Build(Scheduler scheduler)
    {
        ...
        scheduler.AddSystem((
            ...
            Query<
                Data<Bounds, InputState, ZIndex, UOInteractionWidget>,
                Filter<With<Gui>, Optional<UOInteractionWidget>>> query) =>
        {
            var b = batch.Value;
            ...
            if (dumbTexture.Value == null)
            {
                ...
                dumbTexture.Value.SetData(new[] { Color.White });
            }
            ...
            foreach ((var bounds, var inputState, var zIndex, var uoWidget) in query)
            {
                var hue = inputState.Ref switch {
                    InputState.Over => new Vector3(0x22, 1, 1f),
                    InputState.Pressed => new Vector3(0x33, 1, 1f),
                    _ => Vector3.UnitZ
                };
                ...
                else
                {
                    var graphic = inputState.Ref switch {
                        InputState.Over => uoWidget.Ref.Over,
                        InputState.Pressed => uoWidget.Ref.Pressed,
        ...
        scheduler.AddSystem((
            World world,
            Query<Data<Bounds, ZIndex, InputState, Children>, Filter<With<Gui>, Optional<Children>>> query,
            ...
            foreach ((var ent, var bounds, var zIndex, var inputState, var children) in query)
            {
                    ...
                    if (selected.Value.Value == ent.Ref.ID && isDragging)
                    {
                        ...
                        if (length > 0)
                        {
                            ...
                            static void moveChildren(World world, ref Children children, ref readonly Vector2 offset)
                            {
                                ...
                                {
                                    ref var childBounds = ref world.Get<Bounds>(child);
                ...
                else if (bounds.Ref.Value.Contains((int)mousePos.X, (int)mousePos.Y))
                {
                    if (!isDragging)
                    {
                        ...
                        newState = InputState.Over;
                    }
                }

                inputState.Ref = InputState.None;
            }
        ...
        scheduler.AddSystem((TinyEcs.World world) =>
        {
            static EntityView basicWidget(World ecs, Rectangle? bounds = null)
                => ecs.Entity()
                    ...
                    .Set(new ZIndex())
                    .Set(InputState.None)
                    .Set(new Bounds() { Value = bounds ?? Rectangle.Empty});
            ...
            var child0 = basicWidget(world)
                .Set(new UOInteractionWidget() { Normal = 0x1589, Pressed = 0x158B, Over = 0x158A });
            var child1 = basicWidget(world)
                .Set(new UOInteractionWidget() { Normal = 0x085d, Pressed = 0x085e, Over = 0x085f });
...
struct Bounds { public Rectangle Value; }

struct ChildOffset { public Vector2 Value; }

struct ZIndex { public float Value; }

struct UOInteractionWidget
{
