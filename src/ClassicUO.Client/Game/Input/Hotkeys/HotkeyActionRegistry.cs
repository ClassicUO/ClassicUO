...
{
    /// <summary>
    /// Owns the default <see cref="HotkeyAction"/> -&gt; <see cref="Action"/>
    /// catalog. Populated at construction with the hardcoded spell / skill /
    /// virtue / etc. handlers. Single responsibility: resolve a
    /// <see cref="HotkeyAction"/> to its delegate.
    /// </summary>
    internal sealed class HotkeyActionRegistry : IHotkeyActionRegistry
    {
        private readonly Dictionary<HotkeyAction, Action> _actions = new Dictionary<HotkeyAction, Action>();

        public HotkeyActionRegistry()
        {
        ...
        public bool TryGetAction(HotkeyAction action, out Action handler) => _actions.TryGetValue(action, out handler);
