...
{
    /// <summary>
    /// Owns the in-memory dictionary of <see cref="ContainerData"/> keyed by
    /// gump graphic and the on-disk <c>containers.txt</c> file. Single
    /// responsibility: load / persist layout entries and serve lookups. No
    /// gump positioning, no world state, no event subscriptions.
    /// </summary>
    internal sealed class ContainerLayoutStore : IContainerLayoutStore
    {
        ...
        {
            // If the server requests a graphic we have no layout for, create a
            // default placeholder so subsequent lookups stay stable.
