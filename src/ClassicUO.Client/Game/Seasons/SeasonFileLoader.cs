...
namespace ClassicUO.Game.Seasons
{
    ...
    internal sealed class SeasonFileLoader : ISeasonFileLoader
    {
        ...
        public void Load(ISeasonAssetTable table)
        {
            ...
            using (StreamReader reader = new StreamReader(_seasonsFile))
            {
                while (!reader.EndOfStream)
                {
                    ...
                    if (!TryParseSeason(seasonLine[0], out Season season))
                    {
                        continue;
                    }
        ...
        private static bool TryParseSeason(string raw, out Season season)
        {
            switch (raw.ToLower())
            {
                case "spring": season = Season.Spring; return true;
                case "summer": season = Season.Summer; return true;
                case "fall": season = Season.Fall; return true;
                case "winter": season = Season.Winter; return true;
                case "desolation": season = Season.Desolation; return true;
            }
