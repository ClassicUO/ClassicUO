...
namespace ClassicUO.Game.UI.Gumps
{
    internal class ShopGump : Gump
    {
        ...
        //        if (fromcliloc)
        //        {
        //            shopItem.SetName(ServiceProvider.Get<AssetsService>().Clilocs.Translate(it.Name, $"\t{it.Amount}\t{it.ItemData.Name}", true));
        //        }
        //    }
        //}
        ...
        public override void OnButtonClick(int buttonID)
        {
            switch ((Buttons)buttonID)
            {
                case Buttons.Accept:
                    ...
                    if (IsBuyGump)
                    {
                        ServiceProvider.Get<PacketHandlerService>().Out.Send_BuyRequest(LocalSerial, items);
                    }
                    else
                    {
                        ServiceProvider.Get<PacketHandlerService>().Out.Send_SellRequest(LocalSerial, items);
                    }
