    ...
    internal class PaperDollGump : TextContainerGump
    {
        ...
        private void BuildGump()
        {
            ...
            var showRacialAbilitiesBook =
                showPaperdollBooks && ServiceProvider.Get<UOService>().Version >= ClientVersion.CV_7000;
            ...
            if (showPaperdollBooks)
            {
                ...
                if (showRacialAbilitiesBook)
                {
                    ...
                    {
                        if (ServiceProvider.Get<GuiService>().GetGump<RacialAbilitiesBookGump>() == null)
                        {
                            ServiceProvider.Get<GuiService>().Add(new RacialAbilitiesBookGump(World, 100, 100));
                        }
                    };
                }
            }
        ...
        )
        {
            if (args.Button == MouseButtonType.Left)
            {
                PartyGump party = ServiceProvider.Get<GuiService>().GetGump<PartyGump>();
        ...
        public override void OnButtonClick(int buttonID)
        {
            ...
            switch ((Buttons)buttonID)
            {
                ...
                case Buttons.Status:
                    ...
                    if (LocalSerial == World.Player)
                    {
                        ServiceProvider.Get<GuiService>().GetGump<BaseHealthBarGump>(LocalSerial)?.Dispose();
                    ...
                    {
                        if (ServiceProvider.Get<GuiService>().GetGump<BaseHealthBarGump>(LocalSerial) != null)
                        {
                            break;
                        }
        ...
        private class EquipmentSlot : Control
        {
            ...
            private class ItemGumpFixed : ItemGump
            {
                    ...
                    : base(gump, item.Serial, item.DisplayedGraphic, item.Hue, item.X, item.Y)
                {
                    ...
                    _rect = ServiceProvider.Get<UOService>().Arts.GetRealArtBounds(item.DisplayedGraphic);
                ...
                public override bool Draw(UltimaBatcher2D batcher, int x, int y)
                {
                    ...
                    ref readonly var artInfo = ref ServiceProvider.Get<UOService>().Arts.GetArt(item.DisplayedGraphic);
