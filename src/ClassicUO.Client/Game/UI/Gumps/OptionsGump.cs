...
namespace ClassicUO.Game.UI.Gumps
{
    internal class OptionsGump : Gump
    {
        ...
        private readonly AssetsService _fileManager;
        ...
        private void BuildCommands()
        {
            ...
            addButton.MouseUp += (sender, e) =>
            {
                ...
                (
                    ...
                    name =>
                    {
                        ...
                        {
                            if (ServiceProvider.Get<GuiService>().DraggingControl != this || ServiceProvider.Get<GuiService>().MouseOverControl != sss)
                            {
                                return;
                            }
                            ...
                            ServiceProvider.Get<GuiService>().AttemptDragControl(macroButtonGump, true);
                        };
                ...
                ServiceProvider.Get<GuiService>().Add(dialog);
            };
            ...
            for (var macro = (Macro?) macroManager.Items; macro != null; macro = (Macro?) macro.Next)
            {
                ...
                nb.DragBegin += (sss, eee) =>
                {
                    ...
                    ServiceProvider.Get<GuiService>().Gumps.OfType<MacroButtonGump>().FirstOrDefault(s => s._macro == m)?.Dispose();
        ...
        public override void OnButtonClick(int buttonID)
        {
            ...
            switch ((Buttons) buttonID)
            {
                ...
                case Buttons.OpenIgnoreList:
                    ...
                    ServiceProvider.Get<GuiService>().GetGump<IgnoreManagerGump>()?.Dispose();
                    // Open new
                    ServiceProvider.Get<GuiService>().Add(new IgnoreManagerGump(World));
                    break;
            }
        }
        ...
        private void SetDefault()
        {
            switch (ActivePage)
            {
                ...
                case 7: // speech
                    ...
                    ServiceProvider.Get<GuiService>().SystemChat.IsActive = !_chatAfterEnter.IsChecked;
        ...
        private void Apply()
        {
            WorldViewportGump vp = ServiceProvider.Get<GuiService>().GetGump<WorldViewportGump>();
            ...
            if (_currentProfile.TopbarGumpIsDisabled != _enableTopbar.IsChecked)
            {
                ...
                {
                    ServiceProvider.Get<GuiService>().GetGump<TopBarGump>()?.Dispose();
                }
                else
                {
            ...
            {
                SkillGumpAdvanced newGump = ServiceProvider.Get<GuiService>().GetGump<SkillGumpAdvanced>();

                if (newGump != null)
                {
                    ServiceProvider.Get<GuiService>().Add(new StandardSkillsGump(World) { X = newGump.X, Y = newGump.Y });
            ...
            {
                StandardSkillsGump standardGump = ServiceProvider.Get<GuiService>().GetGump<StandardSkillsGump>();

                if (standardGump != null)
                {
                    ServiceProvider.Get<GuiService>().Add(new SkillGumpAdvanced(World) { X = standardGump.X, Y = standardGump.Y });
            ...
            if (_currentProfile.ShowHouseContent != _showHouseContent.IsChecked)
            {
                ...
                ServiceProvider.Get<PacketHandlerService>().Out.Send_ShowPublicHouseContent(_currentProfile.ShowHouseContent);
            }
            ...
            if (_use_old_status_gump.IsChecked != _currentProfile.UseOldStatusGump)
            {
                ...
                if (status != null)
                {
                    if (_currentProfile.StatusGumpBarMutuallyExclusive)
                        status.Dispose();
                    ServiceProvider.Get<GuiService>().Add(StatusGumpBase.AddStatusGump(World, status.ScreenCoordinateX, status.ScreenCoordinateY));
                }
            }
            ...
            if (_statusGumpBarMutuallyExclusive.IsChecked != _currentProfile.StatusGumpBarMutuallyExclusive)
            {
                ...
                if (active && StatusGumpBase.GetStatusGump() != null && ServiceProvider.Get<GuiService>().GetGump<BaseHealthBarGump>(World.Player) is {} bar)
                    bar.Dispose();
            }
            ...
            if (_currentProfile.ChatFont != _fontValue)
            {
                ...
                ServiceProvider.Get<GuiService>().SystemChat.TextBoxControl.Font = _fontValue;
            }
            ...
            var counterGump = ServiceProvider.Get<GuiService>().GetGump<CounterBarGump>();
            ...
            if (updateHealthBars)
            {
                ...
                {
                    List<HealthBarGump> hbgstandard = ServiceProvider.Get<GuiService>().Gumps.OfType<HealthBarGump>().ToList();

                    foreach (HealthBarGump healthbar in hbgstandard)
                    {
                        ServiceProvider.Get<GuiService>().Add(new HealthBarGumpCustom(World, healthbar.LocalSerial) { X = healthbar.X, Y = healthbar.Y });
                ...
                {
                    List<HealthBarGumpCustom> hbgcustom = ServiceProvider.Get<GuiService>().Gumps.OfType<HealthBarGumpCustom>().ToList();

                    foreach (HealthBarGumpCustom customhealthbar in hbgcustom)
                    {
                        ServiceProvider.Get<GuiService>().Add(new HealthBarGump(World, customhealthbar.LocalSerial) { X = customhealthbar.X, Y = customhealthbar.Y });
            ...
            var infoBarGump = ServiceProvider.Get<GuiService>().GetGump<InfoBarGump>();

            if (_currentProfile.ShowInfoBar)
            {
                if (infoBarGump == null)
                {
                    ServiceProvider.Get<GuiService>().Add(new InfoBarGump(World) { X = 300, Y = 300 });
                }
                else
                {
            ...
            if ((byte) _containersScale.Value != containerScale || _currentProfile.ScaleItemsInsideContainers != _containerScaleItems.IsChecked)
            {
                ...
                ServiceProvider.Get<GuiService>().ContainerScale = containerScale / 100f;
                _currentProfile.ScaleItemsInsideContainers = _containerScaleItems.IsChecked;

                foreach (ContainerGump resizableGump in ServiceProvider.Get<GuiService>().Gumps.OfType<ContainerGump>())
                {
            ...
            if (_currentProfile.BackpackStyle != _backpackStyle.SelectedIndex)
            {
                ...
                ServiceProvider.Get<GuiService>().GetGump<PaperDollGump>(World.Player.Serial)?.RequestUpdateContents();
        ...
        private class FontSelector : Control
        {
            ...
            public FontSelector(int max_font, int current_font_index, string markup)
            {
                ...
                {
                    if (assetsService.Fonts.UnicodeFontExists(i))
                    {
                        Add
                        (
