        ...
        protected override void OnMouseUp(int x, int y, MouseButtonType button)
        {
            if (button == MouseButtonType.Left)
            {
                ...
                if (
                    ...
                    && !cursorService.GameCursor.ItemHold.IsFixedPosition
                )
                {
                    if (_myBox != null && _myBox.Bounds.Contains(x, y))
                    {
                        ref readonly var artInfo = ref uoService.Self.Arts.GetArt(cursorService.GameCursor.ItemHold.DisplayedGraphic);
                        x -= _myBox.X;
                        y -= _myBox.Y;
                        ...
                        GameActions.DropItem(cursorService.GameCursor.ItemHold.Serial, x, y, 0, ID1);
                    }
                }
            ...
            if (ServiceProvider.Get<UOService>().Self.Version >= ClientVersion.CV_704565)
            {
                ...
                void OnTextChanged(object sender, EventArgs e)
                {
                    ...
                    if (entry != null)
                    {
                        ...
                        if (send)
                        {
                            ServiceProvider.Get<PacketHandlerService>().Out.Send_TradeUpdateGold(
                                ID1,
                                my_gold_entry,
                                my_plat_entry
                            );
