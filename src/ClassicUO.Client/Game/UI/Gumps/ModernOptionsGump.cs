...
{
    internal partial class ModernOptionsGump : Gump
    {
        ...
        public ModernOptionsGump() : base(0, 0)
        {
            ...
            mainContent.AddToLeft(SidebarCategoryButton(lang.GetTazUO.ButtonTazUO, (int)PAGE.TUOOptions, TazUOConstants.ICON_TAZUO));
        ...
        private void BuildInfoBar()
        {
                ...
                foreach (var child in content.Children)
                {
                    if (child is ScrollArea scrollArea)
                    {
                        ...
                        foreach (var scrollChild in scrollArea.Children)
                        {
                            ...
                            {
                                control.Y = yOffset + 170;
                                yOffset += control.Height; // Ajuste o espaÃ§amento conforme necessÃ¡rio
                                content.ForceSizeUpdate();
                            }
                        }
                        content.ForceSizeUpdate();
            ...
            int infoBarItemCount = 0;
            foreach (var child in content.LeftArea.Children)
            {
                if (child is InfoBarBuilderControl)
                    infoBarItemCount++;
            }
            if (infoBarItemCount > 10)
                content.LeftArea.ToggleScrollBarVisibility(true);
        ...
        private void BuildDust765()
        {
            ...
            content.BlankLine();
            content.AddToRight(new CheckboxWithLabel("Scale monsters (non-humanoid): select a monster on map to show [âˆ’] [+]", 0, profile.ScaleMonstersEnabled, (b) =>
            {
        ...
        private class SettingsOption
        {
            public SettingsOption(string optionLabel, Control control, int maxTotalWidth, PAGE optionsPage, int x = 0, int y = 0)
            {
                ...
                ScrollArea scrollArea = new ScrollArea(
                    ...
                    FullControl.Width,
                     scrollAreaHeight,
                    FullControl.Height // Definir uma altura fixa para a Ã¡rea de rolagem
                ...
                // Substituir FullControl por ScrollArea para ter a rolagem disponÃ­vel
