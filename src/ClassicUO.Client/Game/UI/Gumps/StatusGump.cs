...
namespace ClassicUO.Game.UI.Gumps
{
    internal abstract class StatusGumpBase : Gump
    {
        ...
        public static StatusGumpBase AddStatusGump(World world, int x, int y)
        {
            ...
            if (ServiceProvider.Get<UOService>().Version < ClientVersion.CV_308Z || ProfileManager.CurrentProfile.UseOldStatusGump)
            {
    ...
    internal class StatusGumpOld : StatusGumpBase
    {
        public StatusGumpOld(World world) : base(world)
        {
            ...
            if (ServiceProvider.Get<UOService>().Version >= ClientVersion.CV_5020)
            {
                Add
                (
    ...
    internal class StatusGumpModern : StatusGumpBase
    {
        public StatusGumpModern(World world) : base(world)
        {
            ...
            if (ServiceProvider.Get<UOService>().Version >= ClientVersion.CV_308Z)
            {
                p.X = 389;
                p.Y = 152;
            ...
            {
                if (ServiceProvider.Get<UOService>().Version == ClientVersion.CV_308D)
                {
