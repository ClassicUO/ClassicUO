...
namespace ClassicUO.Game.UI.Controls
{
    internal class StbTextBox : Control, ITextEditHandler
    {
        ...
        public MultilinesFontInfo? CalculateFontInfo(string text, bool countret = true)
        {
            if (IsUnicode)
            {
                return ServiceProvider.Get<UOService>().Self.FileManager.Fonts.GetInfoUnicode
                (
                    _rendererText.Font,
                    text,
                    text.Length,
            ...
            return ServiceProvider.Get<UOService>().Self.FileManager.Fonts.GetInfoASCII
            (
                _rendererText.Font,
                text,
                text.Length,
        ...
        private void Sanitize(ref string text)
        {
            if ((_fontStyle & FontStyle.Fixed) != 0 || (_fontStyle & FontStyle.Cropped) != 0 || (_fontStyle & FontStyle.CropTexture) != 0)
            {
                ...
                if (realWidth > _rendererText.MaxWidth)
                {
                    ...
                    //                              ? ServiceProvider.Get<UOService>().Self.FileManager.Fonts.GetInfoUnicode(
                    //                                                                    _rendererText.Font,
                    //                                                                    text,
                    //                                                                    text.Length,
                    //                                                                    _rendererText.Align,
                    ...
                    //                                                                    realWidth
                    //                                                                   )
                    //                              : ServiceProvider.Get<UOService>().Self.FileManager.Fonts.GetInfoASCII(
                    //                                                                  _rendererText.Font,
                    //                                                                  text,
                    //                                                                  text.Length,
                    //                                                                  _rendererText.Align,
                ...
                case SDL.SDL_Keycode.SDLK_RETURN:
                    if (IsEditable)
                    {
                        ...
                        else
                        {
                            ...
                            if (ServiceProvider.Get<GuiService>().SystemChat != null && ServiceProvider.Get<GuiService>().SystemChat.TextBoxControl != null && IsFocused)
                            {
                                if (!IsFromServer || !ServiceProvider.Get<GuiService>().SystemChat.TextBoxControl.IsVisible)
                                {
                                    OnFocusLost();
                                    OnFocusEnter();
                                }
                                else if (ServiceProvider.Get<GuiService>().KeyboardFocusControl == null || ServiceProvider.Get<GuiService>().KeyboardFocusControl != ServiceProvider.Get<GuiService>().SystemChat.TextBoxControl)
                                {
