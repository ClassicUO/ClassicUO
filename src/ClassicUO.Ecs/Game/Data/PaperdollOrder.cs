...
// =============================================================================
//  PaperdollOrder.cs  (ECS port of ClassicUO.Client/Game/Data/PaperdollOrder.cs)
...
//       specific equipped item graphics (the AnimID, not the world tile graphic).
//    3. Draw each layer's gump in that order — later index = painted on top.
//
//  The tables drive both the paperdoll gump (PaperdollPlugin) and the in-world
//  views (WorldRenderingPlugin.RenderEquipment). Base table T2 is the default
//  order and matches the old paperdoll _layerOrder.
//
//  ECS note: the entity-coupled helpers from the legacy file (GraphicsFromEntity /
//  BuildInWorld, which read GameObjects.Item) are NOT ported. Callers fill the
//  per-layer AnimID span from their own data source (EquipmentSlots + tile data)
//  and call Build -> Filter -> (ApplyDirectionCloak for in-world).
// =============================================================================
