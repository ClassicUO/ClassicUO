...
export function getClayReconciler(events: EventManager) {
  ...
  const shouldSetTextContent = (type: ClayElementNames, props: Props) => {
    ...
    // Fold a single string/number child into the text/label node itself rather
    // than spawning a child text-fragment. A fragment is non-interactive, so an
    // interactive <Text onClick> was left an empty-text parent — a zero-size,
    // unclickable box (dead page-swap buttons). Folding gives the interactive
    // node real glyphs + a real hitbox. Mixed/element children still fragment.
    if (t === 'text' || t === 'label') {
