...
// A single string/number child is the text node's own value (react-dom semantics).
// Anything else (element children, arrays) renders via child text-fragments.
const stringChild = (children: unknown): string =>
...
export function createElement(type: string, props: unknown, id: number): UINode {
  return P.match<ClayElementPropTypes, UINode>({
    ...
    .with({ type: 'text' }, (data) => {
      ...
      // constrains the actual wrap width. `inline` opts out (hug content) so a
      // row of labels lays out side-by-side instead of each taking 100% width.
      if (!data.props.size && !data.props.inline) node.width = grow();
      // A plain-string child lives ON this node (so an interactive <Text onClick>
      // carries its own glyphs + a real hitbox); only mixed/element children fall
      // through to child text-fragments. See shouldSetTextContent.
      return {
        id,
        node,
        text: text(stringChild(data.props.children), data.props.style),
        interactive: hasEventProp(data.props),
        movable: data.props.movable ?? false,
        z: zIndexOf(data.props.floating),
      };
