...
export interface TextProps extends BaseElementProps {
  ...
  padding?: ClayPadding;
  size?: ClaySize;
  // Hug the text content (auto width) instead of the default grow-to-parent.
  // Default text grows to its parent's width so Clay wraps long copy; an inline
  // label in a row must NOT grow (each 100%-wide sibling overflows the row, so
  // only the first shows). Single-line labels don't need wrapping anyway.
  inline?: boolean;
}
