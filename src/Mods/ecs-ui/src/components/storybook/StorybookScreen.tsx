...
export const StorybookScreen: React.FC<StorybookScreenProps> = ({ onBack }) => {
  const [selectedSection, setSelectedSection] = React.useState<StorybookSectionTitle>('Display');

  const { title, description, Stories } = sections.find((section) => section.title === selectedSection)!;
  ...
  return (
    ...
    >
      ...
      >
        <Text inline style={TextStyle.large}>
          Categories:
        </Text>
        {sections.map((section) => (
          <Text
            ...
            style={{
              ...TextStyle.large,
              textColor: selectedSection === section.title ? Colors.gold : Colors.lightGray,
            }}
          >
            {section.title}
          </Text>
