...
{
    public delegate bool SendPacketFilter(ref Span<byte> message);

    public sealed class NetClient : INetworkClient
    {
        ...
        /// <summary>
        /// Optional filter invoked before sending each packet. Return false to suppress the send.
        /// Set by the host application to wire up plugin packet interception.
        /// </summary>
        public SendPacketFilter SendFilter { get; set; }
        ...
        public void Send(Span<byte> message, bool ignorePlugin = false, bool skipEncryption = false)
        {
            ...
            if (!ignorePlugin)
            {
                ...
                if (filter != null && !filter(ref message))
                {
                    return;
                }
            }
