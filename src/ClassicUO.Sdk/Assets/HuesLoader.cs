    ...
    public sealed class HuesLoader : UOFileLoader
    {
        ...
        public void CreateShaderColors(uint[] buffer)
        {
            ...
            for (int r = 0; r < len; r++)
            {
                for (int y = 0; y < 8; y++)
                {
                    for (int x = 0; x < 32; x++)
                    {
                        buffer[idx++] = ColorConverter.Color16To32(HuesRange[r].Entries[y].ColorTable[x]) | 0xFF_00_00_00;
        ...
        public uint GetPolygoneColor(ushort c, ushort color)
        {
            if (color != 0 && color < HuesCount)
            {
                ...
                return ColorConverter.Color16To32(HuesRange[g].Entries[e].ColorTable[c]);
            }
        ...
        public uint GetUnicodeFontColor(ushort c, ushort color)
        {
            ...
            return ColorConverter.Color16To32(c);
        }
        ...
        public uint GetColor(ushort c, ushort color)
        {
            if (color != 0 && color < HuesCount)
            {
                ...
                return ColorConverter.Color16To32(HuesRange[g].Entries[e].ColorTable[(c >> 10) & 0x1F]);
            }

            return color != 0 ? ColorConverter.Color16To32(color) : ColorConverter.Color16To32(c);
        }
        ...
        public uint GetPartialHueColor(ushort c, ushort color)
        {
            if (color != 0 && color < HuesCount)
            {
                ...
                int g = color >> 3;
                int e = color % 8;
                uint cl = ColorConverter.Color16To32(c);
                ...
                if (R == G && R == B)
                {
                    cl = ColorConverter.Color16To32(HuesRange[g].Entries[e].ColorTable[(c >> 10) & 0x1F]);
                }
