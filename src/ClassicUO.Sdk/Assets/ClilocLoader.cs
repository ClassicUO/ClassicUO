...
namespace ClassicUO.Sdk.Assets
{
    public sealed class ClilocLoader : UOFileLoader
    {
        ...
        void ReadCliloc(string path)
        {
            ...
            var output = buf[3] == 0x8E /*|| FileManager.Version >= ClientVersion.CV_7010400*/ ? Decompressor.Bwt(buf) : buf;
        ...
        {
            _entries.TryGetValue(number, out var text);
        ...
        public string GetString(int number, bool camelcase, string replace = "")
        {
            ...
            {
                s = TextUtilities.CapitalizeAllWords(s);
            }
        ...
        {
            var baseCliloc = GetString(clilocNum);
            ...
            var sb = new ValueStringBuilder(baseCliloc.AsSpan());
            {
                ...
                while (pos < sb.Length)
                {
                    ...
                    if (a.Length > 1)
                    {
                        ...
                        {
                            if (_entries.TryGetValue(clil, out var value) && !string.IsNullOrEmpty(value))
                            {
                                a = value.AsSpan();
                            }
                        }
                ...
                if (capitalize)
                {
                    baseCliloc = TextUtilities.CapitalizeAllWords(baseCliloc);
                }
