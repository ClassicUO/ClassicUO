...
namespace ClassicUO.Sdk.Assets
{
    public unsafe sealed class AnimationsLoader : UOFileLoader
    {
        ...
        private void LoadUop()
        {
            ...
            */
            ...
            foreach (var entry in animSeq.Entries)
            {
                ...
                //     Decompressor.Zlib(buf.AsSpan(0, entry.Length), dbuf.AsSpan(0, entry.DecompressedLength));

                //     reader = new StackDataReader(dbuf.AsSpan(0, entry.DecompressedLength));
                // }

                var reader = new StackDataReader(animSeq.Unpack(in entry));
        ...
        )
        {
            ...
            var fakeIdx = new UOFileIndex(file, index.Position, (int)index.Size, (int)index.UncompressedSize, index.CompressionType);
            var reader = new StackDataReader(file.Unpack(in fakeIdx));
        ...
        )
        {
            ...
            while (header != 0x7FFF7FFF && reader.Position < reader.Length)
            {
                ...
                for (int k = 0; k < runLength; ++k, ++block)
                {
                    ...
                    if (!alphaCheck || val != 0)
                    {
                        data[block] = ColorConverter.Color16To32(val) | 0xFF_00_00_00;
                    }
                    else
                    {
                        data[block] = 0;
