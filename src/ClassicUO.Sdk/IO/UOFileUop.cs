...
namespace ClassicUO.Sdk.IO
{
    ...
    public sealed class UOFileUop : UOFile
    {
        ...
        public byte[] Unpack(ref readonly UOFileIndex index)
        {
            Seek(index.Offset, SeekOrigin.Begin);
            var cbuf = new byte[index.Length];
            Read(cbuf);

            if (index.CompressionFlag >= CompressionType.Zlib)
            {
                var dbuf = new byte[index.DecompressedLength];
                Decompressor.Zlib(cbuf, dbuf);

                if (index.CompressionFlag == CompressionType.ZlibBwt)
                {
                    dbuf = Decompressor.Bwt(dbuf);
                }
