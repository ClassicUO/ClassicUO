...
namespace ClassicUO
{
    unsafe class GameController : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            Plugin.ProcessDrawCmdList(ref ptr, ref length);
            HandleCmdList(ptr, length, _resources);


            UpdateWindowCaption(gameTime);
        }

        private Dictionary<IntPtr, GraphicsResource> _resources = new Dictionary<IntPtr, GraphicsResource>();
        ...
        private void HandleCmdList(IntPtr ptr, int length, Dictionary<IntPtr, GraphicsResource> resources)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                switch (cmd.type)
                {
                    ...
                    case CMD_BLEND_STATE:
                        ref var blend = ref cmd.blend_state;

                        break;

                    case CMD_RASTERIZE_STATE:

                        ref var rasterize = ref cmd.rasterize_state;

                        break;

                    case CMD_STENCIL_STATE:

                        ref var stencil = ref cmd.stencil_state;

                        break;

                    case CMD_SAMPLER_STATE:

                        ref var sampler = ref cmd.sampler_state;
